...
# ---------- Helpers ----------
GOAL_LINE = re.compile(r"^- \[(?: |x)\]\s+([DMY]-(?:\d{3}|\d{8,14}[A-Z]?))\s+(.*)$", re.I)
...
def next_id(scope):
    letter = {"daily":"D","monthly":"M","yearly":"Y"}[scope]
    ts = dt.datetime.now(ET).strftime("%Y%m%d%H%M")  # YYYYMMDDHHMM
    base = f"{letter}-{ts}"
    def exists(gid):
        # item file?
        if os.path.exists(os.path.join(ITEMS, f"{gid}.md")):
            return True
        # in masters?
        masters = (DAILY, MONTHLY, YEARLY)
        pat = re.compile(rf"^- \[(?: |x)\]\s+{re.escape(gid)}\b", re.M|re.I)
        return any(pat.search(read_text(m) or "") for m in masters)
    gid = base
    suffix = 0
    while exists(gid):
        suffix += 1
        gid = base + chr(ord('A') + (suffix-1) % 26)  # D-YYYY... + A/B/...
    return gid
...
def iterate_once():
    ...
    for bucket, label in [(d, "Daily"), (m, "Monthly"), (y, "Yearly")]:
        ...
        for g in bucket:
            ...
            # drop steps that repeat summary/original verbatim
            clean_sum = (it.get("summary") or "").strip().lower()
            clean_org = (it.get("original") or "").strip().lower()
            new_steps = [s for s in new_steps if (s or "").strip() and (s.strip().lower() not in {clean_sum, clean_org})]
...
def goal_page(gid):
    ...
    html = render_template("goal.html", title=f"{gid.upper()}", goal=item)
